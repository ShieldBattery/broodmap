...
The baked tables (implemented, phase 3) are indexed by **CHK unified tile id**, not megatile id
— CV5's tile-id -> megatile mapping is folded in at bake time, which is what makes the runtime
path truly zero-asset (a megatile-indexed table would still need CV5 at render time). Per
tileset: one `u8` palette index per tile id, 13 creep palette indices (creep group `cv5[1]`,
slots 0-12), and the 256-color WPE palette — ~21-34 KB each, ~264 KB committed total
(`broodmap-render/src/minimap/tables/*.bin`, via `include_bytes!`). Generated by the hidden
`broodmap-cli gen-minimap-tables` subcommand against a real install (`build_minimap_table` is
the pure core), with an env-gated drift test byte-comparing a fresh jungle generation against
the committed blob — a color table derived from the art, not the art itself.

Runtime (`render_minimap`/`render_chk_minimap`, `MinimapOptions`): `Chk` in, image out, nothing
fetched — usable on a server or in WASM. The creep substitution uses a fixed per-cell
coordinate hash approximating BW's RNG distribution (~4% variant slots 6-12, else uniform 0-5;
determinism is load-bearing, renders must be byte-reproducible). On top of the terrain colors:
unit/THG2-unit-sprite dots in resolved player colors (doodad sprites never drawn), resources
highlighted in SC:R's neutral cyan (toggleable), start locations per the same options as the
preview renderer (`Sprite` degrades to `ColorBlock` — no art exists in this path). `GameData`
is an *optional* input: with it, dots are sized from `units.dat` placeboxes and melee's
start-area clearing applies; without it, dots are 1x1 tiles and the data-free parts of melee
filtering (player-owned and hallucinated drops) still apply. `px_per_tile` (1-8, default 1 =
BW-native; the CLI `minimap` subcommand defaults to 4 for eyeballing) bounds the output.
